...
import { getTrades, verifyContract, buildConfirmTxn, submitSignedTxn } from '../services/api';
...
export default function VerifierDashboard({ address }) {
  ...
  const handleApprove = async (tradeId) => {
    try {
      ...
      const res = await verifyContract(tradeId, true);
  ...
  const handleAction = async (action, tradeId) => {
    if (action === 'confirm') {
      // Open the inline verification panel — just scroll to it
