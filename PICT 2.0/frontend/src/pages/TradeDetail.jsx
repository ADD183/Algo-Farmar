...
import {
  getTrade, acceptContract, buildFundTxn, buildConfirmTxn,
  buildDisputeTxn, buildRefundTxn, submitSignedTxn,
  verifyContract, withdrawFunds, voteDispute,
} from '../services/api';
import { signTransaction, signTransactionGroup } from '../services/wallet';
...
export default function TradeDetail({ address, role }) {
  ...
  const handleFund = async () => {
    try {
      setProcessing(true);
      const res = await buildFundTxn(id);
      const txns = res.data.data.unsigned_txns;
      const signed = await signTransactionGroup([txns.payment_txn, txns.app_call_txn]);
      await submitSignedTxn(id, signed[0]);
  ...
  const handleVerifyApprove = async () => {
    try {
      setProcessing(true);
      const res = await verifyContract(id, true);
      const unsignedTxn = res.data.data.unsigned_txn;
      if (unsignedTxn) {
        const signed = await signTransaction(unsignedTxn);
        await submitSignedTxn(id, signed);
      }
  ...
  const handleWithdraw = async () => {
    try {
      setProcessing(true);
      const res = await withdrawFunds(id);
      const unsignedTxn = res.data.data.unsigned_txn;
      if (unsignedTxn) {
        const signed = await signTransaction(unsignedTxn);
        await submitSignedTxn(id, signed);
  ...
  const handleDispute = async () => {
    try {
      const res = await buildDisputeTxn(id);
  ...
  const handleRefund = async () => {
    try {
      const res = await buildRefundTxn(id);
      const signed = await signTransaction(res.data.data.unsigned_txn);
      await submitSignedTxn(id, signed);
